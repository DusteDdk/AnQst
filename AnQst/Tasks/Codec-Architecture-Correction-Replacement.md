...
This already exists in AnQst's architecture and must remain separate from emission. :contentReference[oaicite:5]{index=5}
...
Where non-boundary structs retain independent wire identities, nested arrays, or nested codec outputs contrary to the one-codec-per-boundary principle. :contentReference[oaicite:7]{index=7}
