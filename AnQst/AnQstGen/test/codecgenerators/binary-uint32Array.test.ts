/**
 * Generator tests for the `binary-uint32Array` base codec emitter.
 * Focus: raw-byte base93 parity, endianness preservation on the current platform,
 * subview/offset safety, corrupted byte-count rejection, and portable C++ wire interoperability.
 */

import test from "node:test";
import assert from "node:assert/strict";
import { emitBase93CppFunctions, emitBase93Decoder, emitBase93Encoder } from "../../src/base93";
import { encoder } from "../../src/codecgenerators/basecodecemitters/binary-uint32Array/encoder";
import { decoder } from "../../src/codecgenerators/basecodecemitters/binary-uint32Array/decoder";
import { assertBase93Alphabet, compileCppProgram, runCppProgram } from "./helpers/emitted-code";

function loadTsCodec(): {
  encode: (value: Uint32Array) => string;
  decode: (encoded: string) => Uint32Array;
...
return {
  encode: encodeBinaryUint32ArrayStandalone,
  decode: decodeBinaryUint32ArrayStandalone,
...
function viewBytes(value: Uint32Array): Uint8Array {
  return new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
}

function copyBytes(value: Uint32Array): Uint8Array {
  return Uint8Array.from(viewBytes(value));
}

function sameElements(actual: Uint32Array, expected: Uint32Array): void {
...
test("binary-uint32Array descriptor exports the spec-facing contract", () => {
  assert.equal(encoder.descriptor, decoder.descriptor);
  assert.equal(encoder.descriptor.codecId, "binary-uint32Array");
  assert.equal(encoder.descriptor.specPath, "RefinedSpecs/Codecs/Binary_uint32Array_Codec.md");
  assert.equal(encoder.descriptor.tsType, "Uint32Array");
  assert.equal(encoder.descriptor.cppType, "QByteArray");
  assert.equal(encoder.descriptor.wireCategory, "binary");
});

test("binary-uint32Array TypeScript codec round-trips representative arrays with wire parity", () => {
  const { encode, decode } = loadTsCodec();
  const cases = [
    new Uint32Array([]),
    new Uint32Array([0]),
    new Uint32Array([1, 2, 3]),
    new Uint32Array([0xffffffff, 0x12345678, 0xabcdef01, 0x00000001]),
    new Uint32Array([0x01020304, 0x05060708, 0x090a0b0c])
  ];
  ...
  for (const sample of cases) {
    ...
    assert.ok(roundTrip instanceof Uint32Array);
...
test("binary-uint32Array preserves raw platform byte order on the wire", () => {
  const { encode, decode, decodeBase93 } = loadTsCodec();
  const sample = new Uint32Array([0x12345678, 0xaabbccdd, 0x000000ff, 0xff000000]);
...
test("binary-uint32Array encoder respects non-zero byteOffset and byteLength for subviews", () => {
  const { encode, decode, decodeBase93 } = loadTsCodec();
  ...
  const sample = new Uint32Array(backing, 4, 3);
  const expectedBytes = copyBytes(sample);
  const wire = encode(sample);
...
test("binary-uint32Array decoder rejects decoded byte counts not divisible by 4", () => {
  const { decode, encodeBase93 } = loadTsCodec();
  const corruptWire = encodeBase93(new Uint8Array([0x12, 0x34, 0x56, 0x78, 0x9a]));
  assert.throws(() => decode(corruptWire), RangeError);
});

test("binary-uint32Array portable C++ wire interop matches TypeScript raw bytes", (t) => {
  ...
  const cpp = [
    ...
    "  if (mode == \"encode\") {",
    "    std::cout << encodeBinaryUint32ArrayStandalone(fromHex(payload));",
    "    return 0;",
    "  }",
    "  if (mode == \"decode\") {",
    "    std::cout << toHex(decodeBinaryUint32ArrayStandalone(payload));",
    "    return 0;",
    "  }",
    "  return 3;",
    "}"
  ...
  const exe = compileCppProgram(t, "binary_uint32array_codec_cpp", cpp);
  if (!exe) return;

  const cases = [
    new Uint32Array([]),
    new Uint32Array([0x12345678]),
    new Uint32Array([0x12345678, 0x90abcdef, 0x000000ff]),
    new Uint32Array([0xffffffff, 0x00000000, 0x01020304, 0xf0f1f2f3])
  ];
