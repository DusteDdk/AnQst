...
export const descriptor: BaseCodecDescriptor = {
  ...
  wireCategory: "binary",
  strategySummary:
    "Base93-encode the raw bytes of the Uint32Array view (respecting byteOffset/byteLength); C++ receives the same raw bytes in QByteArray."
};
...
export const encoder: BaseCodecEncoderEmitter = {
  ...
  emitTsEncoder(): string {
    return [
      emitStrategyComment("AnQst Uint32Array encoder (raw-byte base93)", [
  ...
  emitCppEncoder(): string {
    return [
      emitStrategyComment("AnQst Uint32Array encoder (QByteArray raw bytes)", [
